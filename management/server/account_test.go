...
func createManager(t testing.TB) (*DefaultAccountManager, error) {
	...
	manager, err := BuildManager(context.Background(), store, NewPeersUpdateManager(nil), nil, "", "netbird.cloud", eventStore, nil, false, MocIntegratedValidator{}, metrics, port_forwarding.NewControllerMock(), settingsManager, permissionsManager, false)
	if err != nil {
		return nil, err
	}
